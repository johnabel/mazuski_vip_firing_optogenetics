...
def format_2pi_axis(ax, x=True, y=False, y0=False):
    """ y0 sets yaxis start at 0"""
    import numpy as np
    if x:
    ...
    if y:
        ...
        ax.set_yticklabels([r'$-\pi$', r'$-\pi/2$','$0$',
                            r'$\pi/2$', r'$\pi$'])
    if y0:
...
def format_npi_axis(ax, n=6, x=True, y=False):
    ...
    if y:
        ax.set_yticks([nn*np.pi for nn in range(n+1)])
        ax.set_ylim([0, n*np.pi])
        ax.set_yticklabels(['$0$', r'$\pi$', r'$2\pi$',
                            r'$3\pi$', r'$4\pi$', '$5\pi$', r'$6\pi$'])
