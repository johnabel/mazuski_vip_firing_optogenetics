...
def find_onset_region(region, x, bin3, sens='hi'):
    """ finds the first peak in bin3 given the sensitivity metric from 
    Leise 2013. """
    specific_region = bin3[region[0]:region[1]]
    specific_x_locs = np.arange(len(specific_region))+region[0]
    specific_times = x[specific_x_locs]
    
    # find peaks
    peaks_loc = signal.argrelextrema(specific_region, np.greater, order=3)[0]
    peaks     = specific_region[peaks_loc]
    rel_peaks = peaks/np.max(peaks)
    ...
    onset_loc = peaks_loc[np.min(np.where(rel_peaks >= sens)[0])]
    onset_time = specific_times[onset_loc]
    return onset_time

def dwt_onset(times, counts):
    """ identifies wheel running onset from times and counts"""
    x = times
    y = counts
    dwt = bl.dwt_breakdown(x, y, wavelet='db4')
    
    bin3 = dwt['components'][3]
    bin6 = dwt['components'][5]
    
    # identify regions of increasing activity
    maxs6 = signal.argrelextrema(bin6, np.greater, order=15)[0]
    mins6 = signal.argrelextrema(bin6, np.less, order=15)[0]
    inc_regions = []
    for mini in mins6[:-1]:
        try:
            maxi = np.min(maxs6[np.where(maxs6>mini)])
    ...
    for region in inc_regions:
        onsets.append(find_onset_region(region,x,bin3,sens='lo'))
...
for path in example_paths:
    times3, counts3 = load_invivo_data(path, binning = 15)
    dwt_res = dwt_onset(times3, counts3)
    plt.plot(dwt_res['onsets']%24, 'o')

plt.figure()
plt.plot(times3, counts3)
plt.plot(times3, dwt_res['dwt']['components'][2])
plt.plot(dwt_res['onsets']-1.25, np.ones(len(dwt_res['onsets'])), 'ko')




times2, counts2 = load_invivo_data(example_path2, binning = 15)
times3, counts3 = load_invivo_data(example_path3, binning = 15)
